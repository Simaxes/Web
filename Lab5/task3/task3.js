function getTime(n) {
	...
	minets = parseInt((n-(hours*3600))/60);
	second = (n-(hours*3600)-(minets*60));
	hours = (hours < 10) ? (result = "0" + hours + ":") : (result += hours + ":");
	minets = (minets < 10) ? (result += "0" + minets + ":") : (result += minets + ":");
	second = (second < 10) ? (result += "0" + second) : (result += second);
	return result;
}
